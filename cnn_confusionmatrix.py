...
from testinginput_cnn import input_testdata
import time
...
testx,testy=input_testdata()  #导入测试数据集
print("Finished")
...
#定义两个placeholder
x = tf.placeholder(tf.float32,[None,1152])  #x列 行不确定
y = tf.placeholder(tf.float32,[None,2])  #是ddos=1.或 不是ddos=0

keep_prob = tf.placeholder(tf.float32)

#创建一个简单的神经网络  包含两个隐藏层
W1 = tf.Variable(tf.truncated_normal([1152,500],stddev=0.1))#二初始值
...
with tf.Session() as sess:
    ...
    test = time.clock()
    #测试评价
    tp, tn, fp, fn = sess.run([tp_op, tn_op, fp_op, fn_op], feed_dict={x: test_X, y: test_y, keep_prob: 1.0})
    ...
    test_time = (time.clock() - test)
    print("Testing： accuracy= " + str(accuracy) + ", recall= " + str(recall) + ", precision= " + str(precision) + ", f1_score= " + str(f1_score))
    print("testing time:" + str(test_time) + "s")  # 印出时间