...
def packet_parse(pcap_file):
    packets = rdpcap("./train_data/"+pcap_file)
    #返回负载特征
    payloads = ""
    packetPayload=[]
...
output.to_csv("./data_payload_raw.csv",index=False)  #把一个DataFrame写入csv文件
# output.to_csv("./data_payload_normalise.csv",index=False)  #把一个DataFrame写入csv文件

def get_payloadFeatures1152():
    start=time.clock()
    i=0
    num = len(pcap_files)
    ...
    for pcap_file in pcap_files:
        ...
        packetPayload,label = packet_parse(pcap_file)  #得到负载和标签
        feature = packetPayload + label
        f = open("./data_payload_raw.csv", 'a')
        f.write(str(feature).strip('[]') + '\n')
        # #归一化
        # feature = minmax_scale(packetPayload).tolist()+label
        # f = open("./data_payload_normalise.csv", 'a')
        # f.write(str(feature).strip('[]') + '\n')
    tran_time=time.clock()-start
    print(tran_time)

get_payloadFeatures1152()